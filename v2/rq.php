...
<?php
	...
	if ($_SERVER['HTTP_HOST']) {
		$cpath = "http://".$_SERVER['HTTP_HOST'].ereg_replace("(.*)/rq.php", "\\1", $_SERVER['PHP_SELF']);
	}
	else {
		$cpath = "http://".$_SERVER['SERVER_NAME'].ereg_replace("(.*)/rq.php", "\\1", $_SERVER['PHP_SELF']);
	}

	$path		= strtolower(trim($_REQUEST['path']));
	$path		= eregi_replace("([^-[:alnum:]_]+)", "", $path);
	$pwd		= trim($_REQUEST['pwd']);
	$mname		= trim(str_replace("_", " ", $_REQUEST['mname']));
	$mname		= strtolower(eregi_replace($master_name_filter, "", $mname));
	...
	if ($path && $pwd && $mname && $mail && $_REQUEST['verify'] && $_REQUEST['terms']) {
		...
		$mail = urlencode($mail);
		@system("wget -q --output-document=/dev/stdout \"http://pjj.cc/newchat.php?nchat={$path}&password={$pwd}&username={$mname}&email={$mail}&login=Tino+Didriksen&pass=YUKzh2SJ\"");
		echo "<br>Should be done now. Check your email...";
/*
		$header = "Chat Request: $path, $mname";
//		$body = "$cpath/newchat.php?username=".urlencode($mname);
		$body = "http://pjj.cc/newchat.php?username=".urlencode($mname);
